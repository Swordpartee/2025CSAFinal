...
public class Encryption {
     ...
     * This implementation avoids unnecessary memory allocations by:
     * 1. Pre-scanning to count delimiters before allocating result array
     * 2. Using System.arraycopy for efficient copying
     * 3. Reusing the original array reference when possible
     * 
     * @param data The byte array to split
     * @param delimiter The delimiter byte
     * @return An array of byte arrays, each containing a segment of the original array
     */
    public static byte[][] splitByteArray(byte[] data, byte delimiter) {
        ...
        // Handle special case: if the array ends with a delimiter, we don't want an empty segment
        ...
        // Special case: no delimiters found
        if (count == 1) {
            // Reuse original array if it has no delimiters
            if (data.length > 0 && data[data.length - 1] != delimiter) {
                result[0] = data;
            } else {
                // Create a copy excluding the trailing delimiter
                result[0] = Arrays.copyOf(data, data.length - 1);
            }
            return result;
        }
        
        // Second pass: split the array
        int start = 0;
        int resultIndex = 0;
        ...
        for (int i = 0; i < data.length; i++) {
            if (data[i] == delimiter) {
                int length = i - start;
                if (length > 0) {
                    result[resultIndex] = new byte[length];
                    System.arraycopy(data, start, result[resultIndex], 0, length);
                    resultIndex++;
                } else if (start == 0 || i > 0 && data[i-1] == delimiter) {
                    // Handle empty segments (consecutive delimiters)
        ...
        // Handle the last segment if it's not empty and doesn't end with delimiter
        if (start < data.length) {
            int length = data.length - start;
            result[resultIndex] = new byte[length];
            System.arraycopy(data, start, result[resultIndex], 0, length);
        } else if (data[data.length - 1] == delimiter) {
